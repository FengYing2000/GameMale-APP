...
## 圖片在另一個子網域，需要第二顆 WebView

版塊圖示、勳章、帖子裡的圖都放在 **`img.gamemale.com`**，跟論壇本體不同來源
（頭像例外，它在 `www` 底下的 `uc_server/avatar.php`）。

所以「用 WebView 發 fetch」對圖片是不夠的：停在 `www` 的頁面對 `img` 發請求
是**跨來源**，而那個網域不送 CORS 標頭，直接被瀏覽器擋掉。症狀非常好認：
**頭像有顯示、其他圖全滅**。

那個子網域同樣被 Cloudflare 擋著，所以也不能退回一般的 HTTP 客戶端。作法是
**每個來源各一顆 WebView**：抓 `img.gamemale.com` 的圖時用一顆停在該來源的
WebView，請求就變成同源。那顆不給使用者看——圖片是靜態資源，Cloudflare
通常會自己放行，不需要互動。

順帶兩個效能／穩定性的細節：
- **探測結果要快取幾秒**。首頁一次二十幾張圖，每張都各自探測一次的話等於
  對同一顆 WebView 併發丟出幾十次 JS 求值，其中一次回傳不如預期那張圖就掛。
- **併發要設上限**（4）。全部一起送反而整批逾時，排隊比較快。

## 帖子圖片會跨來源轉址，要用導覽而不是 fetch

帖子裡的圖是 `www` 的 `forum.php?mod=image&aid=…`，它 **302 轉到
img.gamemale.com**。從停在 www 的頁面 `fetch` 過去，瀏覽器跟著跨來源的轉址
之後會做 CORS 檢查，一樣被擋；而且 `redirect: 'manual'` 拿到的是不透明回應，
連轉址目標都讀不到。

**導覽沒有這個限制**：讓 WebView 直接開那個網址，跟完轉址後文件的 origin
就落在圖片那一邊，這時 `fetch(location.href)` 就是同源的。

代價是一次只能跑一張（導覽會把整顆 WebView 帶走），所以用一把鎖排隊，
而且只在一般的 fetch 先失敗時才走這條。版塊圖示那種直接指向 img 子網域的
不受影響，走原本的路就好。

## 背景很久之後不能相信舊畫面

WebView 上會留著背景前渲染好的舊論壇頁面——DOM 裡有論壇連結，探測會說
「是論壇」，於是驗證頁一開就自己關掉，而請求依然失敗。更糟的是那次假成功
會啟動「剛解過就不再彈」的冷卻，害使用者按重試也叫不出驗證頁。
...
Cloudflare 擋著的那段期間，畫面上的圖片會全部失敗，而**失敗狀態是記在元件
裡的**。下拉刷新只重抓資料，圖片網址沒變、元件被重用，所以不會重試——
使用者看到的是「驗證完了，可是左上角的頭像還是空的，只能重開 App」。

所以驗證通過與傳輸方式改變時要遞增 `imageRetryTick`，`NetImage` 監聽它並
把失敗的重載一次。另外每張圖自己也會先重試一次再放棄：Cloudflare 剛通過的
那幾秒常有零星失敗，一次就判死刑的話那些圖要等到下一個信號才會回來。

## 在線會員有兩種形狀

首頁的 `#online` 區塊**收合時（以及未登入時）只吐總人數與歷史紀錄**：

    总计 3193 人在线 - 最高记录是 45510 于 2026-9-4.

沒有會員／訪客的細分，也沒有名單。展開且**登入**之後才會有完整資料。
所以介面不能無條件顯示「會員 N · 訪客 N」——那時 N 都是 0，是錯的
（用 `OnlineInfo.hasBreakdown` 判斷）。

名單要 `forum.php?showoldetails=yes`，而且那是使用者在論壇上的偏好設定。
做成使用者真的展開卡片時才抓，不必為它每次都拉一份大頁面。

## 別在啟動時鎖死走瀏覽器

上次被 Cloudflare 擋過就在啟動時 `forceBrowser()` 的話，會鎖在慢的那條路，
要等十分鐘的探測才回得去直連——論壇一關掉驗證，使用者每次啟動都白白慢
...
`getBytes` / `getAbsoluteBytes`（驗證碼、圖片位元組）走的是 dio，**跟 `get`
是兩條路**。只導了 `get` 的話，被擋著時 dio 拿回來的是攔截頁的 HTML，
`Image.memory` 當然畫不出來——而**驗證碼失效正好發生在使用者要重新登入的
時候**，卡在那裡就完全沒救了。

## 導覽要用獨立的 WebView

帖子圖的導覽後備會把整個頁面帶走，其他正在用同一顆 WebView `fetch()` 的
圖片會當場斷掉。症狀是「有些版塊圖示出得來、有些出不來」，而且每次失敗的
還不一樣。導覽另開一顆，抓圖那顆才能穩定停在圖片來源上。
...
驗證是逐站設定的。實測就出現過「**www 被擋、img 子網域沒擋**」的組合
（2026-09-07）。用一個全域的 `_preferBrowser` 的話，圖片會跟著繞進 WebView，
白白慢好幾倍——而那正是最需要快的東西。

`Api._blocked` 改成 `Map<host, 發現時間>`，每個主機各自判斷、各自定期回頭
探直連。介面用 `Api.hostNeedsBrowser(url)` 決定某張圖要不要繞。

順帶一提，實測那個外掛**只擋訪客**：登入之後所有請求都直連，整套繞道機制
只是待命。所以正常情況下速度跟沒有驗證時一樣。

## 登出一定要連 WebView 的 cookie 一起清

App 的連線和 WebView **各有一份 cookie**。登出時只清 App 那份的話，WebView
仍是登入狀態，於是全部走樣：

- 被驗證擋著時走的是 WebView，它抓回來的是**舊帳號**的頁面 → 資料看起來沒更新
- 驗證頁一打開就自己關掉 → 它探測的是 WebView，那邊一切正常，使用者看不到彈窗
- **驗證碼最糟**：拿到的是 WebView 那個 session 的碼，就算圖出來了也對不上

所以 `signOut()` 要呼叫 `BrowserFetch.clearCookies()`（清掉並重載所有 WebView），
並 `Api.resetTransport()` 讓下一個請求重新判斷擋不擋、也把「剛解過就不再彈」
的冷卻清掉——登出之後使用者馬上就需要驗證頁跳出來。
