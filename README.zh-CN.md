...
| **页首的私信数会被自己清掉** | 用户瞄一眼消息列表，论坛就把页首那个数字归零，但对话本身还是未读——后台通知只看页首就会整批漏掉，要用每则对话自己的未读数 |
...
动机是侧载 IPA 治不好的两个毛病——**7 天到期要重签**，以及
**推送一定要付费开发者账号**。iOS 16.4 起支持 Web Push，
主屏幕 App 收得到真推送，而且 VAPID 密钥是自己生成的，不用跟 Apple 申请。

目前只做到「证明推送会响」：RFC 8291 消息加密与 RFC 8292 VAPID 都自己实现
（pub.dev 上没有 Dart 包），用 RFC 官方测试向量验证过，加上一个会逐项显示
前置条件的诊断页。论坛功能还没接进去。
...
- **推送通知** — 原生自签版需要 APNs 证书，拿不到；网页版做得出来（自写 Web Push），但目前以 `PUSH_ENABLED=false` 关闭
