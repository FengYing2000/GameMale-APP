...
[![Tests](https://img.shields.io/badge/测试-265%20项-4CAF50)](#测试策略)
...
<table>
<tr><td width="90"><b>浏览</b></td><td>版块列表（收藏的版块／订阅的专辑／子版块展开／版主与简介）· 主题列表（全部／最新／热门／热帖／精华）· 投票与悬赏筛选 · 排序与时间范围 · 主题分类 · 帖子内页 · 附件 · 淘帖（推荐／所有／我的专辑＋专辑内页）· 分页</td></tr>
<tr><td><b>互动</b></td><td>回复 · 引用回复 · 发表主题 · 编辑自己的帖子 · 收藏主题／版块（黄星切换）· 顶／踩 · 淘帖入专辑 · 使用道具（提升泵／亮色刷）· 举报 · 评分（快速评分／自动跳过缺项）· 投票</td></tr>
<tr><td><b>社区</b></td><td>私信（气泡对话）· 通知（两层分类）· 个人资料（角色组／勋章／管理版块／已加入群组）· 个人空间七个子页 · 加好友 · 打招呼 · 记录广场（时间／楼中楼回复／逐条回复）</td></tr>
<tr><td><b>搜索</b></td><td>帖子 · 日志 · 相册 · 群组 · 用户 · 本版搜索 · 高级搜索（全文／作者／主题范围／特殊主题／时间／排序）</td></tr>
<tr><td><b>账号</b></td><td>账密登录（图形验证码／安全提问）· 注册问答 · 登出 · 每日签到（排行榜／补签卡）· 我的收藏（帖子／版块／群组／日志／相册）／主题／回复 · 回帖记录</td></tr>
<tr><td><b>体验</b></td><td>深／浅色 · 六色强调色 · RPG 风格提示气泡 · 繁简即时切换（换语言整页重绘）· 流量控制 · 表情选择器 · 外部链接跳转提示 · 回帖奖励横幅 · 已回帖标记 · Lucide 图标 · 内建浏览器（带登录状态）· 图片长按菜单 · 楼中楼 · 固定分页栏（切页回顶）· 下拉刷新</td></tr>
</table>
...
| 文件 | 内容 | 数量 |
|---|---|---|
| `parse_test.dart` | 用真实抓下来的页面验证每个选择器 | 204 |
| `pages_test.dart` | 每页 pump 起来 + 离线行为 | 31 |
| `s2t_test.dart` | 繁简转换的每一类判断 | 18 |
| `render_test.dart` | 真实帖子 HTML 丢进 PostBody 确认画得出来 | 12 |
| `live_test.dart` | 对真实论坛的端到端（需 cookie，CI 自动跳过） | 46 |

```bash
flutter test                        # 265 项离线测试
flutter analyze                     # 零问题
```
