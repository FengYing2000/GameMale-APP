...
[![測試](https://img.shields.io/badge/自動測試-372_項-4CAF50)](docs/DEVELOPING.md)
...
<td>版塊列表（可展開子版塊、點圖示看版主與版規）、主題列表可依<b>最新／熱門／精華</b>切換，也能只看投票或懸賞。帖子內頁支援分頁、附件、<b>樓中樓</b>，還能快速跳到指定樓層。首頁底部有<b>在線會員</b>，點名字可直接看他的資料。</td>
</tr>
<tr>
<td><b>✍️ 發言</b></td>
<td>回覆、引用回覆、發表新主題、編輯自己的帖子。內建<b>表情選擇器</b>與 BBCode 快捷鍵，回帖有獎勵時會直接顯示橫幅。已經回過的主題會標「已回」。回覆頁會顯示<b>字數</b>、不到論壇下限先提醒；主題關閉或達每日回帖上限時直接顯示論壇給的原因，送出後<b>確認真的有發出去</b>才算成功。</td>
</tr>
<tr>
<td><b>⭐ 互動</b></td>
...
<tr>
<td><b>🎨 個人化</b></td>
<td>深色／淺色主題、<b>彩虹旗六色</b>強調色任選、繁簡體即時切換、圖片載入策略（一律載入／只在 Wi-Fi／手動）。長按圖片可以儲存、分享或複製連結。原生版支援<b>多帳號切換</b>（可選記住密碼，存在裝置的安全區、不上傳）。</td>
</tr>
</table>
...
論壇有時會開啟人機驗證來擋機器人。**網頁版與 App 版現在都會自動處理**，
多數情況你不用做任何事就能照常瀏覽。

少數情況下原生版會跳出一頁驗證，照著點一下「我是人類」、等幾秒，它就會
自己關閉、恢復正常。
</details>
...
```bash
flutter test                                        # 299 項
cd packages/gm_api && dart test                     # 41 項
cd pwa/server && dart test                          # 32 項
```
