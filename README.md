...
| **頁首的私訊數會被自己清掉** | 使用者瞄一眼訊息列表，論壇就把頁首那個數字歸零，但對話本身還是未讀——背景通知只看頁首就會整批漏掉，要用每則對話自己的未讀數 |
...
<tr><td><b>社群</b></td><td>私訊（氣泡對話）· 通知（兩層分類）· 個人資料（角色組／勳章／管理版塊／已加入群組）· 個人空間七個子頁 · 加好友 · 打招呼 · 記錄廣場（時間／樓中樓回覆／逐則回覆）· 新提醒／新私訊紅點 · 背景檢查＋本地通知 · 提醒可回招呼／忽略 · 打招呼 14 種動作＋留言</td></tr>
...
| 背景通知 | 系統決定何時喚醒，強制關閉就停 | WorkManager 照週期跑，可靠得多 |
...
動機是側載 IPA 治不好的兩個毛病——**7 天到期要重簽**，以及
**推播一定要付費開發者帳號**。iOS 16.4 起支援 Web Push，
主畫面 App 收得到真推播，而且 VAPID 金鑰是自己產的，不用跟 Apple 申請。

目前只做到「證明推播會響」：RFC 8291 訊息加密與 RFC 8292 VAPID 都自己實作
（pub.dev 上沒有 Dart 套件），用 RFC 官方測試向量驗證過，加上一個會逐項顯示
前置條件的診斷頁。論壇功能還沒接進去。
...
- **推播通知** — 原生自簽版需要 APNs 憑證，拿不到；網頁版做得出來（自寫 Web Push），但目前以 `PUSH_ENABLED=false` 關閉
